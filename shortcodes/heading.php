...
if ( ! class_exists( 'IG_Heading' ) ) {
	...
	class IG_Heading extends IG_Pb_Element {
			...
			$this->items = array(
				'content' => array(
					...
					array(
						...
						'tooltip' => __( 'Tag Description', IGPBL )
					),
					array(
						'name'    => __( 'Text', IGPBL ),
						'id'      => 'text',
				...
				'styling' => array(
					...
					array(
						...
						'tooltip'         => __( 'Bottom Border Description', IGPBL ),
						'container_class' => 'combo-group',
					),
					array(
				...
				if ( $value != '' ) {
					if ( $key == 'font_face_type' ) {
						if ( $value == __( 'Standard fonts', IGPBL ) || $value == 'standard fonts' ) {
							...
							$font_face_value = $arr_params['font_face_value'];
							$script = '<script type="text/javascript">(function ($) {$(document).ready(function () {
	$(\'head\').append("<link rel=\'stylesheet\' href=\'http://fonts.googleapis.com/css?family=' . $font_face_value . '\' type=\'text/css\' media=\'all\' />");
});})(jQuery)</script>';
							$font_style = 'font-family:' . $arr_params['font_face_value'];
						}
