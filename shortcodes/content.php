...
if ( ! class_exists( 'IG_Content' ) ) {
	...
	class IG_Content extends IG_Pb_Element {
		...
		public function element_shortcode_( $configs, $atts = null, $content = null, $contentclips = false ) {
			...
			$limit          = isset($ig_cl_limit) ? intval( $ig_cl_limit ) : null;
			...
			if ( isset( $contentclips_page ) ) {
				...
				$limit = intval( $ig_cl_limit ) - $items_per_page * ( $contentclips_page - 1 );
				...
				if ( in_array( $ig_cl_source, $arr_post_types ) ) {
					...
					if ( ! empty( $item_filter ) ) {
						...
						$sql     = $wpdb->prepare(
							...
							WHERE term_rel.term_taxonomy_id IN ( $item_filter )",
							null
						);
						...
						foreach ( $objlist as $i => $item ) {
							...
							$sqlx      = $wpdb->prepare(
								...
								WHERE term_taxonomy_id = {$item->term_taxonomy_id}",
								null
							);
